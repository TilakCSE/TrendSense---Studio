...
export function EtherealShadow({
    sizing = 'fill',
    color = '#001a11', // A deep, nearly black Charleston tone for the waves
    ...
    // FIX: Wait for client-side mounting to avoid Next.js hydration wipe
    ...
    }, [isMounted, animationEnabled, animationDuration, hueRotateMotionValue]);

    // Fallback for SSR
    if (!isMounted) return <div className={className} style={{ width: "100%", height: "100%", backgroundColor: '#00291C', ...style }}>{children}</div>;

    return (
        <div className={className} style={{ overflow: "hidden", position: "relative", width: "100%", height: "100%", backgroundColor: '#00291C', ...style }}>
            
            {/* SVG Filter & Blob */}
            <div style={{ position: "absolute", inset: -displacementScale, filter: animationEnabled ? `url(#${id}) blur(15px)` : "none", opacity: 0.7, zIndex: 0 }}>
                {animationEnabled && (
            ...
            {noise && noise.opacity > 0 && (
                <div style={{
                    ...
                    backgroundSize: noise.scale * 200, backgroundRepeat: "repeat", opacity: noise.opacity / 2
                }} />
            )}

            {/* Content sits ABOVE the shadow */}
