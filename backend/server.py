...
# ─── Analytics Endpoints (P0) ─────────────────────────────────────────────────
# These endpoints serve the pre-computed static_analytics.json generated by
# data/generate_static_analytics.py.  Zero runtime Parquet I/O — fast & safe.

# In-process cache so the JSON file is read from disk only once per server boot.
_analytics_cache: dict | None = None

STATIC_ANALYTICS_PATH = os.path.join(
    os.path.dirname(__file__), "..", "data", "static_analytics.json"
)


def _load_analytics() -> dict:
    """Load and cache static_analytics.json on first access."""
    global _analytics_cache
    if _analytics_cache is not None:
        return _analytics_cache

    if not os.path.exists(STATIC_ANALYTICS_PATH):
        raise FileNotFoundError(
            "static_analytics.json not found. "
            "Run `python data/generate_static_analytics.py` first."
        )

    with open(STATIC_ANALYTICS_PATH, "r", encoding="utf-8") as f:
        _analytics_cache = json.load(f)

    logger.info("✅ [Analytics] static_analytics.json loaded into memory.")
    return _analytics_cache
...
def get_analytics_summary():
    """
    Returns KPI-card metrics for v2 and v3.
    Data is sourced from the pre-computed static_analytics.json file.
    """
    try:
        ...
        return JSONResponse(content={
            ...
            "v2": { "summary": data.get("v2", {}).get("summary") },
            "v3": { "summary": data.get("v3", {}).get("summary") },
        })
...
def get_analytics_distributions():
    """
    Returns pre-aggregated distribution data for v2 and v3.
    """
    try:
        data = _load_analytics()
        return JSONResponse(content={
            "status": "success",
            "v2": {
                "correlation_matrix": data.get("v2", {}).get("correlation_matrix"),
                "engagement_stats":   data.get("v2", {}).get("engagement_stats"),
            },
            "v3": {
                "histogram":          data.get("v3", {}).get("histogram"),
                "correlation_matrix": data.get("v3", {}).get("correlation_matrix"),
                "engagement_stats":   data.get("v3", {}).get("engagement_stats"),
                "scatter_sample":     data.get("v3", {}).get("scatter_sample"),
            }
        })
    except FileNotFoundError as e:
        raise HTTPException(status_code=503, detail=str(e))
    except Exception as e:
        logger.error(f"Analytics distributions failed: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=str(e))